...
1. Create a `.env` file with API keys:
   ...
   XAI_API_KEY=xai-...
   ```
2. Install Node.js dependencies: `npm install openai dotenv node-fetch`
3. Install Python dependencies: `pip install pandas matplotlib seaborn scipy numpy`

### Execution Order
```bash
# 1. Generate the prompt matrix
node 01_generate_prompts.js

# 2. Build JSON job list
node 02_build_generation_jobs.js

# 3. Generate images (run both, order doesn't matter)
node 03_gpt_run_generation.js
node 03_grok_run_generation.js

# 4. Audit skin tones (run for each provider)
node 04_audit.js --provider openai
node 04_audit.js --provider xai

# 5. Run full analysis
python 05_run_full_analysis.py
```

All generation scripts are **resumable** — interrupted runs can be restarted and will pick up where they left off.
